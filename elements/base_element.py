...
class BaseElement:
    ...
    def click(self, nth: int = 0, **kwargs: str | int) -> None:
        with allure.step(f'Clicking on {self.type_off} "{self.name}"'):
            locator = self.get_locator(nth, **kwargs)
            locator.click()

    def check_visible(self, nth: int = 0, **kwargs: str | int) -> None:
        with allure.step(f'Checking that {self.type_off} "{self.name}" is visible'):
            locator = self.get_locator(nth, **kwargs)
            expect(locator).to_be_visible()

    def check_have_text(self, text: str, nth: int = 0, **kwargs: str | int) -> None:
        with allure.step(f'Checking that {self.type_off} "{self.name}" has text: "{text}"'):
            locator = self.get_locator(nth, **kwargs)
